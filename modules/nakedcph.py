...
class NakedCPH(BotBase):
    def __init__(self, task, proxies, task_id, settings, task_data):
        BotBase.__init__(self, 'NakedCPH', task_id, settings)
    ...
    def monitor(self):
        ...
        while True:
            ...
            if response.status_code != 200:
                ...
                try:
                    productData = json.loads(soup.find('script', {'class':'item-data-script is-gtag-event-tracking'}).text)
                except Exception as e:
                ...
                try:
                    self.prodName = productData['display_product_name']
                except:
                    self.prodName = '?'

                try:
                    self.prodPrice = productData['price_current']
                except:
                    self.prodPrice = '?'
                
                try:
                    self.prodImage = productData['img_url']
                except:
                    self.prodImage = '?'
                ...
                try:
                    sizes = soup.find('select', {'id':'product-form-select'}).findChildren()
                except:
    ...
    def atc(self):
        while True:
            ...
            if self.captchaActive:
                ...
                captchaToken = self.getCaptchaToken('6LeNqBUUAAAAAFbhC-CS22rwzkZjr_g4vMmqD_qo')
                data['g-recaptcha-response'] = captchaToken

            try:
                atc = self.session.post('https://www.nakedcph.com/en/cart/add', json=data)
            except Exception as e:
            ...
            if atc.status_code != 200:
                ...
                self.webhook.cartCookie('png.state', self.cartCookie, 'NakedCPH', self.task['Profile Name'], self.sku, self.prodName, self.prodPrice, self.prodImage, self.elapsedTime, self.captchaActive)
